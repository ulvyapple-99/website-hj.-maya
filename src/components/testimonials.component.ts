    ...
    <section class="py-20 bg-gray-50 border-t border-gray-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12">
           <h2 class="text-3xl font-bold mb-4 text-brand-brown">Kata Pelanggan Kami</h2>
           <p class="text-gray-500">Apa kata mereka tentang kelezatan Sate Maranggi Hj. Maya?</p>
        </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
           @for (review of config().testimonials; track $index) {
             <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition duration-300 relative group">
                <div class="text-4xl text-brand-orange/20 absolute top-4 right-6 font-serif">"</div>
                
                <div class="flex items-center gap-1 mb-4">
                  @for (star of [1,2,3,4,5]; track star) {
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" 
                      [class.text-brand-orange]="star <= review.rating"
